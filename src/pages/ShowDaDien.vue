  ...
  <div class="schedule-page">
    <section class="schedule-page__header">
      <div class="schedule-page__inner">
        <h2 class="schedule-page__title">Các Show đã diễn</h2>
        <p class="schedule-page__subtitle">Danh sách các lịch diễn đã hoàn thành</p>
      </div>
    </section>

    <section class="schedule-page__content">
      <div v-if="loading && schedules.length === 0" class="state-message">Đang tải danh sách show đã diễn...</div>
      <div v-else-if="errorMessage && schedules.length === 0" class="state-message state-message--error">{{ errorMessage }}</div>
      <div v-else-if="!loading && schedules.length === 0" class="state-message">Không có show nào ở trạng thái "Đã diễn".</div>
...
const fetchSchedules = async () => {
  ...
  errorMessage.value = ''
  try {
    const res    = await fetch(API_ENDPOINTS.GET_SHOW_DA_DIEN, { method: 'GET', redirect: 'follow' })
