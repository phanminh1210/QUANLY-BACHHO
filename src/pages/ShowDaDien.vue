  ...
  <div class="schedule-page">
    <section class="schedule-page__header">
      <div class="schedule-page__inner">
        <p class="schedule-page__subtitle">Danh sách các show đã diễn</p>
      </div>
    </section>
...
const fetchSchedules = async () => {
  ...
  try {
    const endpoint = (API_ENDPOINTS as any).GET_SHOW_SAP_TOI || API_ENDPOINTS.GET_SHOW_DA_DIEN
    const res = await fetch(endpoint, { method: 'GET', redirect: 'follow' }).then(safeJson)
    ...
    if (res && (res.success !== false || res.code === 200 || res.status === 'success')) {
      ...
      if (allSchedules.value.length === 0) {
        errorMessage.value = 'Không có show nào trong danh sách.'
      }
    } else {
...
const goToRegister = (item: Schedule) => router.push({ name: 'DangKyShow', query: { showId: String(item.id) } })
